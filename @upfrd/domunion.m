...
% DOMUNION   Define UPFRDs on a common domain
%
% Let A and B be UPFRDs.  If A depends on independent variables (X,Y)
% and B depends on independent variables (X,Z) then
% [Aext,Bext]=domunion(A,B) returns UPFRDs Aext and Bext that have
...
if ~flg
   ...
   % Get domains for each UPFRD
   dcell = cell(nin,1);
   for i=1:nin
      varargin{i} = upfrd( varargin{i} );
   ...
   % Expand each input UPFRD to be constant along new IV dimensions
   ...
   for i=1:nin
      ...
      Aext = upfrd(Adata,Udom);
      varargout{i} = Aext;
   end
else
   % Get public domains for each UPFRD
   dcell = cell(nin,1);
   for i=1:nin
      varargin{i} = upfrd( varargin{i} );
   ...
   % Expand each inM1put UPFRD to be constant along new IV dimensions
   varargout = cell(nin,1);
   for i=1:nin
      A = varargin{i};
      Aidx = idxcell{i};
