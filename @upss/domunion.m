...
% DOMUNION   Define UPSSs on a common domain
%
% Let A and B be UPSSs.  If A depends on independent variables (X,Y)
% and B depends on independent variables (X,Z) then
% [Aext,Bext]=domunion(A,B) returns UPSSs Aext and Bext that have
...
if ~flg
   ...
   % Get domains for each upss
   dcell = cell(nin,1);
   for i=1:nin
      varargin{i} = upss( varargin{i} );
   ...
   % Expand each input upss to be constant along new IV dimensions
   ...
   for i=1:nin
      ...
      Aext = upss(Adata,Udom);
      varargout{i} = Aext;
   end
else
   % Get public domains for each upss
   dcell = cell(nin,1);
   for i=1:nin
      varargin{i} = upss( varargin{i} );
